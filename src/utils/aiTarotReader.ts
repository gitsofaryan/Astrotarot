...
Though the celestial channels are momentarily veiled, the cards ${cards
      .map((c) => c.name)
      .join(
        ", "
      )} have already begun weaving their mystical tapestry around your soul's journey.

✨ **The Past Whispers**: ${cards[0].name} speaks of ${
      cards[0].keywords[0]
    } that has shaped your spiritual foundation. This energy has been your hidden teacher, preparing you for this moment of divine seeking.

🌙 **The Present Pulses**: ${
      cards[1].name
    } illuminates your current path with ${
      cards[1].keywords[0]
    }. The universe asks you to embrace this energy as your guiding star through the cosmic currents of now.

⭐ **The Future Beckons**: ${cards[2].name} promises ${
      cards[2].keywords[0]
    } will manifest in your journey ahead. Trust that the cosmic forces are aligning to bring this blessed energy into your reality.
...
export const getCardContext = (
  ...
  const elementalEnergy = card.suit
    ? ` The ${card.suit} energy brings elemental wisdom of ${getSuitElement(
        card.suit
      )}.`
    : "";

  return `${card.name} ${positionMeanings[position]}, radiating ${card.keywords
    .slice(0, 2)
    .join(" and ")}.${elementalEnergy}`;
};
...
export const generateAffirmation = (cards: TarotCard[]): string => {
  const affirmations = [
    ...
    `The cosmic forces of ${cards
      .map((c) => c.keywords[0])
      .join(", ")} flow through my being, aligning me with my highest destiny.`,
    ...
    `I am divinely guided by the sacred trinity of ${cards
      .map((c) => c.name)
      .join(
        ", "
      )}, each card a beacon lighting my path to spiritual fulfillment.`,
  ];
...
export const getCardSynergy = (cards: TarotCard[]): string => {
  ...
  if (majorCount === 3) {
    synergy =
      "A powerful trinity of Major Arcana reveals this is a time of profound spiritual transformation and karmic significance.";
  } else if (majorCount === 2) {
    synergy =
      "Two Major Arcana cards indicate significant life lessons and spiritual growth are at hand.";
  } else if (majorCount === 1) {
    synergy =
      "One Major Arcana card provides the spiritual foundation for practical manifestation.";
  } else {
    synergy =
      "The Minor Arcana speaks to everyday magic and practical spiritual application.";
  }

  if (uniqueSuits.length === 1 && suits.length > 1) {
    synergy += ` The repeated ${suits[0]} energy amplifies ${getSuitElement(
      suits[0]
    )}.`;
